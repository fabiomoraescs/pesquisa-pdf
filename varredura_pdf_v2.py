...
def salvar_excel_completo(
    ...
    """
    Mantém as abas auxiliares úteis da versão anterior e altera
    a estrutura da aba principal "Ocorrencias".

    Aba principal:
      ID livro | Termo | Unidade | Capítulo | Seção | Subseção |
      Página | Tipo de ocorrência | Contexto sociológico | Descrição

    Preenchimento automático:
      ID livro, Termo, Página, Contexto sociológico e Descrição

    Preenchimento manual:
      Unidade, Capítulo, Seção, Subseção e Tipo de ocorrência
    """
    ...
    if df_completo.empty:
        df_saida = pd.DataFrame(
            columns=colunas_principais
        )
    else:
        df_saida = pd.DataFrame({
            "ID livro":
                df_completo["ID livro"],

            "Termo":
                df_completo["Termo"],

            # Campos destinados ao preenchimento manual.
            "Unidade":
                "",
            ...
            # O tipo detectado pelo programa continua existindo
            # internamente para a aba "Resumo por tipo", mas a
            # coluna principal fica vazia para classificação manual.
            "Tipo de ocorrência":
                "",
    ...
    # As abas-resumo continuam sendo calculadas com os dados
    # internos da varredura, como na versão anterior.
    (
        resumo_termo,
        resumo_tipo
    ) = criar_resumos(
    ...
    with pd.ExcelWriter(
        ...
        # Abas auxiliares mantidas, sem categoria do termo.
...
def main():
    ...
    for numero, pdf in enumerate(
        ...
        # Mantém o nome original do PDF (sem a extensão .pdf)
        # e acrescenta _v2 ao final do arquivo Excel.
        arquivo_individual = (
            PASTA
            / f"resultado_{pdf.stem}_v2.xlsx"
        )
